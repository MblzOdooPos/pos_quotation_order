odoo.define('pos_quotation_order.QuotationScreen', function (require) {
    'use strict';
    ...
    class QuotationScreen extends IndependentToOrderScreen {
        constructor() {
            super(...arguments);
        ...
        getSelectedSyncedOrder() {
            // console.log('synced order', this._state.ui.filter)
            if (this._state.ui.filter == 'SYNCED') {
        ...
        getFilteredOrderList() {
            // console.log('deberia ahora cargar las ordnees para renderixad', this.env.pos.quotations)
            // console.log('filter', this._state.ui.filter)
            if (this._state.ui.filter == 'SYNCED') {
        ...
        getTime(order) {
            return order.time_order;
        }
        getTotal(order) {
            return this.env.pos.format_currency(order.amount_total);
        }
        getCustomer(order) {
        ...
        _setOrder(order) {
            ...
            if (!current_order) {
                var table = this.env.pos.tables_by_id[1];
                this.env.pos.set_table(table);
                this.showScreen('ProductScreen');
                current_order = this.env.pos.add_new_order()
                // console.log('orders', )
                
            }
            console.log('original_order', order);
            current_order.origin_quotation = order.id;
            const partner = this.env.pos.db.get_partner_by_id(order.partner_id[0]);
            if (partner && current_order) {
                if (current_order.get_client() !== partner) {
                    current_order.set_client(partner);
                    current_order.set_pricelist(
            ...
            for (let l = 0; l < order.lines.length; l++) {
                ...
                // console.log('product_id', line, line.product_id)
                var product = this.env.pos.db.get_product_by_id(line.product_id);
                // console.log('crearr lineas', current_order, product)
                current_order.add_product(product, {
                    ...
                    merge: false,
                    // extras: {
                    //     ref_payment_data: ref_payment_data,
                    //     return_ref: this.props.ref,
    ...
    QuotationScreen.template = 'QuotationScreen';
    QuotationScreen.defaultProps = {
    ...
    Registries.Component.add(QuotationScreen);
